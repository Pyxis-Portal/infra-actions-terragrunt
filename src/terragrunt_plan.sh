...
function terragrunt_plan {
	...
	echo "plan: info: planning Terragrunt configuration in ${tf_working_dir}"
	planOutput=$(${tf_binary} plan -detailed-exitcode -input=false ${*} 2>&1)
	planExitCode=${?}
	...
	if [ ${planExitCode} -eq 0 ]; then
		echo "plan: info: successfully planned Terragrunt configuration in ${tf_working_dir}"
		echo "${planOutput}"
		echo
	...
	if [ ${planExitCode} -ne 0 ]; then
		echo "plan: error: failed to plan Terragrunt configuration in ${tf_working_dir}"
		echo "${planOutput}"
		echo
	fi

	# Comment on the pull request if necessary.
	if [ "$GITHUB_EVENT_NAME" == "pull_request" ] && [ "${tf_comment}" == "1" ] && ([ "${planHasChanges}" == "true" ] || [ "${planCommentStatus}" == "Failed" ]); then
		planCommentWrapper="#### \`${tf_binary} plan\` ${planCommentStatus}
